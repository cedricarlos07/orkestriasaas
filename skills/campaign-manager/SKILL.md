...
5. Google: `create_search_campaign` / `create_pmax_campaign` (AdLoop when alc_ linked)
6. Writes: dry_run=true then dry_run=false

## Notes

Campaigns stay PAUSED until the owner confirms. Meta via [adkit](https://github.com/jatinjain25/adkit). Google via AdLoop Cloud when connected.
