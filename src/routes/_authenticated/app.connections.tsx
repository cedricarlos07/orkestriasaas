        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 space-y-3">
            <div>
              ...
              <p className="text-[12px] text-ink-soft">
                Google passe par AdLoop self-hosted sur le serveur (MCC agence). Connectez OAuth pour cibler un compte
                client précis, ou laissez le compte par défaut du MCC.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {googleSetup?.adloopHealth.ok ? (
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[12px] font-medium text-emerald-700">
                  <CheckCircle2 className="h-3.5 w-3.5" /> AdLoop self-hosted OK
                </span>
              ) : googleSetup?.adloopConfigured ? (
                <span className="inline-flex items-center gap-1 text-[12px] text-amber-700">
                  <AlertCircle className="h-3.5 w-3.5" />
                  {googleSetup.adloopHealth.error ?? "AdLoop non prêt — lancez adloop init sur le serveur"}
                </span>
              ) : (
                <span className="text-[12px] text-amber-700">AdLoop désactivé côté serveur</span>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-2 border-t border-line/50 pt-3">
              {googleSetup?.oauthConnected ? (
                <>
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[12px] font-medium text-emerald-700">
                    <CheckCircle2 className="h-3.5 w-3.5" /> OAuth · {googleSetup.account ?? "compte lié"}
                  </span>
                  <button
                    type="button"
                    className="chip-ghost text-[12px]"
            ...
            <ul className="divide-y divide-line/60">
              {Object.values(CONNECTORS)
                ...
                .map((cfg) => {
                  ...
                  const googleReady = isGoogle && (googleSetup?.adloopHealth.ok || googleSetup?.oauthConnected);
                            ...
                            {isMeta && metaReallyConnected
                              ...
                              : isMeta && metaSetup?.tokenError
                                ...
                                : isGoogle && googleSetup?.oauthConnected
                                  ? `${accountLabel} · AdLoop self-hosted`
                                  : isGoogle && googleSetup?.adloopHealth.ok
                                    ? "AdLoop self-hosted (compte MCC par défaut)"
                                    : connected
                                      ? accountLabel
                                      : configured
                                        ? "Disponible"
