...
import { billAllOrgsPreviousMonth } from "@/lib/stripe/billing";

/**
 * Monthly commission invoices for previous calendar month.
 * Auth: Authorization: Bearer $CRON_SECRET
 */
export const Route = createFileRoute("/api/cron/commission-bill")({
  server: {
    handlers: {
...
async function handle(request: Request): Promise<Response> {
  ...
  try {
    const result = await billAllOrgsPreviousMonth();
